...
- No `opencode.local.json` created (gitignored by design — machine-specific)
- No secrets set up
...
- `~/.dotfiles` README documents the symlink pattern for the other 4 tools;
  ...
  (matches existing dotfiles pattern, requires `opencode.local.json` +
...
1. **Shared base + local layer.** `opencode.json` (tracked, identical across
   machines, NO model/provider info) + gitignored `opencode.local.json`
...
4. **`autopilot` primary agent** — `mode: primary` (hidden from being
   ...
   except its own `docs/superpowers/**` audit trail, drives a custom
...
4. Write the design doc via `superpowers:brainstorming` →
   `docs/superpowers/specs/YYYY-MM-DD-opencode-config-redesign-design.md`
...
6. Only after implementation + verification: wire `~/.dotfiles/bootstrap.sh`,
   ...
   `opencode.local.json` + `secrets/*`, and — only if the user explicitly asks
   — commit.
