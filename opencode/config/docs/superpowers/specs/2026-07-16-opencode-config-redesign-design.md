...
**Staging location:** `~/.dotfiles/opencode/config/` (vendored base from
...
**In scope:** the global config tree under `~/.dotfiles/opencode/config/` —
...
- Deploying to `~/.config/opencode`: no symlink, no `bootstrap.sh` change, no
  `opencode.local.json`, no secrets population, no commit — all deferred to a
  later step (§14).
...
**23 custom agents + built-in `plan`.**
...
Added by this user (4): `web-fast-context` (already migrated to the live
...
**openai spread:** `devops`, `skill-smith` on `gpt-5.6-terra`; `webresearcher`
...
`bash: false`, `edit` denied except its own `docs/superpowers/**` audit trail,
...
`opencode.local.json` + `secrets/` present-but-gitignored in the target. **Not
built now.** This work produces a complete, correct config tree under
`~/.dotfiles/opencode/config/` and leaves the live `~/.config/opencode` and
`bootstrap.sh` untouched. No `opencode.local.json`, no secrets, no symlink, no
