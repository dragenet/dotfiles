...
Keep every graphify artifact in the project's `.agents/graphify-out/`
directory. Build or rebuild with `graphify extract . --out .agents`; never use
...
1. Inspect `.agents/graphify-out/` first. When `.agents/graphify-out/graph.json` exists and a
...
Use the `graphify` skill for extraction, merge, cluster, export, cleanup, and
reporting procedures, substituting `.agents/graphify-out/` for every output
path. Report missing worker chunks or failed extraction honestly; do not
substitute broad source-file reads for graph results.
