...
**Goal:** Turn the raw-vendored `~/.dotfiles/opencode/` tree into a complete, loadable, cost/performance-optimized OpenCode global config per the approved design (`.agents/superpowers/specs/2026-07-16-opencode-config-redesign-design.md`).

**Architecture:** A single shared `opencode.jsonc` (no models) + submodule-vendored superpowers/skills + a per-machine `opencode.local.jsonc` model layer, a 23-agent roster, six plugins, a graphify orchestrator/worker pair, and CLI-based context7. Each phase leaves the config loadable.
...
- Config root is `~/.dotfiles/opencode/` — the future `~/.config/opencode` symlink target. All relative paths below are from this root unless stated.
- **Do NOT** touch the live `~/.config/opencode`, `~/.dotfiles/bootstrap.sh`, or `infra-flux`. No symlink, no `opencode.local.jsonc` with real secrets, no `git commit` unless the user explicitly asks.
...
- Superproject git repo is `~/.dotfiles` (remote `github.com:dragenet/dotfiles`); submodules register in `~/.dotfiles/.gitmodules` with `opencode/<name>` paths.
...
- [ ] **Step 2: Add all six submodules under `opencode/`**

Run (from `~/.dotfiles`):
```bash
git submodule add https://github.com/obra/superpowers.git opencode/superpowers
git submodule add https://github.com/anthropics/skills.git opencode/anthropics-skills
git submodule add https://github.com/cloudflare/skills.git opencode/cloudflare-skills
git submodule add https://github.com/google-labs-code/stitch-skills.git opencode/stitch-skills
git submodule add https://github.com/VoltAgent/awesome-agent-skills.git opencode/awesome-agent-skills
git submodule add https://github.com/avivsinai/jenkins-cli.git opencode/jenkins-cli
```
Expected: each clones into `opencode/<name>` and appends an entry to `~/.dotfiles/.gitmodules`.
...
```bash
git -C ~/.dotfiles add .gitmodules opencode/superpowers opencode/anthropics-skills opencode/cloudflare-skills opencode/stitch-skills opencode/awesome-agent-skills opencode/jenkins-cli
...
Produces the two per-machine `opencode.local.jsonc` templates and the rewritten `model-selection.md`. These are gitignored templates (committed as `*.example` so they're tracked without leaking machine specifics).

### Task 2.1: Create the personal model layer template

**Files:**
- Create: `opencode.local.personal.example.jsonc`
...
Create `opencode.local.personal.example.jsonc` with the §3 personal column verbatim:
```json
{
...
**Files:**
- Create: `opencode.local.work.example.jsonc`
...
- [ ] **Step 2: Add `opencode.local.jsonc` + `secrets/kilo.org` to gitignore coverage**

Run: `grep -q 'opencode.local.jsonc' .gitignore && echo "already ignored" || echo "opencode.local.jsonc" >> .gitignore`
...
- [ ] **Step 3:** With a throwaway `OPENCODE_CONFIG=$PWD/opencode.local.personal.example.jsonc`, run `OPENCODE_CONFIG=... opencode models >/dev/null 2>&1; echo "load exit=$?"` → Expected: `0` (config loads without a parse/agent-resolution error).
