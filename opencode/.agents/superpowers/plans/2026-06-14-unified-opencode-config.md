...
**Goal:** Make `~/.config/opencode-priv` a shared base that runs identically on both the private and work machines, with machine-specific providers/models isolated to a thin `opencode.local.jsonc` layer loaded via direnv.

**Architecture:** A single shared `opencode.json` (private architecture + work MCP/agents ported in additively, no `model`/`provider` keys) is deep-merged at runtime with a gitignored, per-machine `opencode.local.jsonc` that sets `model`/`small_model`, the `provider` block, and per-agent opus overrides. `.envrc` (direnv) exports `OPENCODE_CONFIG` automatically. Secrets stay per-machine via `{file:secrets/*}`.
...
| `opencode.local.jsonc` | Per-machine: `model`/`small_model`, `provider` block, opus overrides for architect/debugger. Gitignored. |
| `.envrc` | `export OPENCODE_CONFIG="$PWD/opencode.local.jsonc"`. Gitignored. |
...
| `.gitignore` | Ignore `opencode.local.jsonc` and `.envrc`. |
...
- Create: `~/.config/opencode-priv/opencode.local.jsonc`
...
- [ ] **Step 4: Create `opencode.local.jsonc` (private machine values)**

Create `~/.config/opencode-priv/opencode.local.jsonc` with exactly:
...
Run: `jq empty opencode.json && jq empty opencode.local.jsonc && echo OK`
Expected: `OK`

- [ ] **Step 6: Commit**

```bash
git add opencode.json opencode.local.jsonc
git commit -m "refactor(config): move model/provider to opencode.local.jsonc layer; relative instructions"
```

Note: `opencode.local.jsonc` is committed here intentionally as a starting template; Task 7 adds it to `.gitignore` and `git rm --cached`s it so future per-machine edits stay local. (If you prefer it never tracked, skip the `git add opencode.local.jsonc` here and create it after Task 7 — but committing the template first gives the work repo a copy to start from.)
...
- [ ] **Step 2: Add `opencode.local.jsonc` and `.envrc` to `.gitignore`**
...
- [ ] **Step 3: Stop tracking `opencode.local.jsonc` (it was committed as a template in Task 1)**

Run:

```bash
git rm --cached opencode.local.jsonc
```

Expected: `rm 'opencode.local.jsonc'` (file stays on disk).

- [ ] **Step 4: Verify both are now ignored**

Run: `git check-ignore opencode.local.jsonc .envrc`
Expected output:
```
opencode.local.jsonc
.envrc
...
```bash
git add .gitignore
git commit -m "chore(config): gitignore opencode.local.jsonc + .envrc; add direnv loader"
```
...
block. Those live in a per-machine `opencode.local.jsonc` (gitignored), which
OpenCode deep-merges on top of the base via the `OPENCODE_CONFIG` env var.

- **Defaults:** `opencode.local.jsonc` sets `model` = sonnet and `small_model` =
  haiku.
- **Per-agent override:** only `architect` and `debugger` get
  ...
  agent, edit one line in `opencode.local.jsonc`.
...
- **Loading:** `.envrc` (direnv) exports
  `OPENCODE_CONFIG="$PWD/opencode.local.jsonc"` on entering the config dir. Run
  `direnv allow .` once per machine. Without direnv, export `OPENCODE_CONFIG`
  manually (absolute path to `opencode.local.jsonc`).
...
- Create: `opencode.local.jsonc` (work values), `.envrc`
...
Apply the same `.gitignore` additions (`opencode.local.jsonc`, `.envrc`,
`.direnv/`).
...
- [ ] **Step 2: Create work's `opencode.local.jsonc` with the work provider/model block**

Create `~/.config/opencode/opencode.local.jsonc` with:
...
```bash
cd ~/.config/opencode
jq empty opencode.json && jq empty opencode.local.jsonc && echo OK
```
Expected: `OK`
...
```bash
cd ~/.config/opencode-priv && OPENCODE_CONFIG="$PWD/opencode.local.jsonc" opencode --version
cd ~/.config/opencode && OPENCODE_CONFIG="$PWD/opencode.local.jsonc" opencode --version
```
...
(Do NOT commit `opencode.local.jsonc`, `.envrc`, or `secrets/*` — they are gitignored. Remove the `opencode.json.prev` backup once verified: `rm opencode.json.prev`.)
...
```bash
diff <(jq -S 'del(.provider)' ~/.config/opencode-priv/opencode.local.jsonc) \
     <(jq -S 'del(.provider)' ~/.config/opencode/opencode.local.jsonc)
```
...
```
Expected: no tracked changes pending; gitignored `opencode.local.jsonc`/`.envrc`/`secrets/*` do not appear.
...
choices isolated to a gitignored, per-machine `opencode.local.jsonc` loaded via
