...
- **Narzędzia:** `read`, `glob`, `grep` (zrozumienie kontekstu) + `write`
  (TYLKO własne artefakty pod `.agents/superpowers/`). **`bash: deny`** (nie
  uruchamia) + **`edit: { "*": "deny", ".agents/superpowers/**": "allow" }`**.
  ...
  gdziekolwiek, a pozwala zapisać artefakty tylko pod `.agents/superpowers/`.
