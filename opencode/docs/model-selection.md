...
`.agents/superpowers/specs/2026-07-16-opencode-config-redesign-design.md`.
...
`opencode.local.jsonc`, deep-merged on top of the base config via the
...
Of the agents on `openai` (personal machine only): `skill-smith` uses
`gpt-5.6-terra`; `webresearcher` uses `gpt-5.6-luna`.
