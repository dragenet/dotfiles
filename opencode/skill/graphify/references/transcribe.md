...
**Strategy:** Read the god nodes from `.agents/graphify-out/.graphify_detect.json` (or the analysis file if it exists from a previous run). You are already a language model — write a one-sentence domain hint yourself from those labels. Then pass it to Whisper as the initial prompt. No separate API call needed.
...
detect = json.loads(Path('.agents/graphify-out/.graphify_detect.json').read_text(encoding=\"utf-8\"))
...
Path('.agents/graphify-out/.graphify_transcripts.json').write_text(json.dumps(transcript_paths, ensure_ascii=False), encoding=\"utf-8\")
print(f'Transcribed {len(transcript_paths)} file(s)', file=sys.stderr)
"
```

After transcription:
- Read the transcript paths from `.agents/graphify-out/.graphify_transcripts.json`
