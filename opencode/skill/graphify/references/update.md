...
```bash
$(cat .agents/graphify-out/.graphify_python) -c "
import sys, json
...
Path('.agents/graphify-out/.graphify_incremental.json').write_text(json.dumps(result, ensure_ascii=False), encoding=\"utf-8\")
...
import json
from pathlib import Path
r = json.loads(Path('.agents/graphify-out/.graphify_incremental.json').read_text(encoding=\"utf-8\"))
Path('.agents/graphify-out/.graphify_detect.json').write_text(json.dumps({
...
result = json.loads(open('.agents/graphify-out/.graphify_incremental.json', encoding='utf-8').read()) if Path('.agents/graphify-out/.graphify_incremental.json').exists() else {}
...
```bash
if [ ! -f .agents/graphify-out/.graphify_extract.json ]; then
    echo '[graphify update] Only deletions -- creating empty extraction for merge.'
    $(cat .agents/graphify-out/.graphify_python) -c "
import json
from pathlib import Path
Path('.agents/graphify-out/.graphify_extract.json').write_text(json.dumps({'nodes':[],'edges':[],'hyperedges':[],'input_tokens':0,'output_tokens':0}), encoding='utf-8')
"
fi
```
...
new_extraction = json.loads(Path('.agents/graphify-out/.graphify_extract.json').read_text(encoding=\"utf-8\"))
incremental = json.loads(Path('.agents/graphify-out/.graphify_incremental.json').read_text(encoding=\"utf-8\"))
...
G = build_merge(
    [new_extraction],
    graph_path='.agents/graphify-out/graph.json',
    prune_sources=prune,
    root='INPUT_PATH',
...
Path('.agents/graphify-out/.graphify_extract.json').write_text(json.dumps(merged_out, ensure_ascii=False), encoding=\"utf-8\")
...
old_data = json.loads(Path('.agents/graphify-out/.graphify_old.json').read_text(encoding=\"utf-8\")) if Path('.agents/graphify-out/.graphify_old.json').exists() else None
new_extract = json.loads(Path('.agents/graphify-out/.graphify_extract.json').read_text(encoding=\"utf-8\"))
...
Before the merge step, save the old graph: `cp .agents/graphify-out/graph.json .agents/graphify-out/.graphify_old.json`
Clean up after: `rm -f .agents/graphify-out/.graphify_old.json`
