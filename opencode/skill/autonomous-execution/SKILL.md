...
- Save design as an artifact (`.agents/superpowers/specs/YYYY-MM-DD-<topic>-design.md`).
...
### 3. PLAN
Produce a plan artifact following `writing-plans` conventions (`.agents/superpowers/plans/YYYY-MM-DD-<feature>.md`). Tasks must be fully specified — subagents will execute without asking you design questions.
...
Then save it to `.agents/superpowers/audits/YYYY-MM-DD-<task>-report.md` (NOT the
project root). Two acceptable ways:
- write it yourself (you have `edit` allowed under `.agents/superpowers/**`), or
